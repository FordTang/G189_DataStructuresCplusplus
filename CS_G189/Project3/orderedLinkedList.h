...
private:
	bool recursiveSearch (nodeType<Type> *searchNode, const Type &searchItem) const;
	  //Function that gets called from search function.
	  //Uses recursion to determine if searchItem exist in list.
      //Postcondition: Returns true if searchItem is in the list, 
      //    otherwise the value false is returned.
};
...
bool orderedLinkedList<Type>::search(const Type &searchItem) const
{
	...
	*/
	...
	return recursiveSearch(first, searchItem);
...
void orderedLinkedList<Type>::mergeLists (orderedLinkedList<Type> &list1, orderedLinkedList<Type> &list2)
{
	...
	for (temp = list1.begin(); temp != list1.end(); ++temp)
	{
		insert(*temp);
	}

	for (temp = list2.begin(); temp != list2.end(); ++temp)
	{
		insert(*temp);
	}
...
template<class Type>
bool orderedLinkedList<Type>::recursiveSearch (nodeType<Type> *searchNode, const Type &searchItem) const
{
	if (searchNode->info == searchItem)
		return true;
	else if (searchNode->info == NULL)
		return false;
	else
		return this->recursiveSearch(searchNode->link, searchItem);
}
