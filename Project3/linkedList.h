...
{
	if (this -> search(item))  //confirms if item is in the list.
	{
		if (first -> info != item)  //checks if item is first; quits if first.
		{
			nodeType<Type> *current; //pointer to traverse the list
			...
			while (current -> info != item)  //finds item in the original list
			{
				trailCurrent = current;
				current = current -> link;
				count ++;
			}
			last = trailCurrent;  //sets last to node before item found
			last -> link = NULL;  //sets last link to NULL
			
			secondList.initializeList();  //Clears secondList
			secondList.first = current;  //protected data member error
			while (current != NULL) //Corrects count number for secondList
			{
				secondList.count++;
				current = current -> link;
			}
			...
			cout << "Item is in the front of the list.  Cancelling divideAt procedure." << endl;
	}
	else
		cout << "Item is not in the list.  Cancelling divideAt procedure." << endl;
}
#endif
